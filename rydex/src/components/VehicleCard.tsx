...
import { Bike, Bus, Car, Truck, Star, Gauge, IndianRupee, Clock, ArrowRight } from 'lucide-react'
...
function VehicleCard({ vehicle, distance, onBook }: { vehicle: IVehicle, distance: number | undefined, onBook: () => void }) {
    const { Icon, label } =
        TYPE_CONFIG[vehicle.type as keyof typeof TYPE_CONFIG]
    ...
    return (
        <motion.div
            ...
            className='relative bg-white border border-zinc-200 rounded-3xl overflow-hidden
        ...
        >
            <div className='relative h-48 bg-zinc-50 flex items-center justify-center overflow-hidden'>
                ...
                <motion.img
                    ...
                    style={{ filter: "drop-shadow(0 8px 34px rgba(0,0,0,0.14))" }}
            ...
            text-white text-[10px] font-bold uppercase tracking-wider px-2.5 py-1.5
            rounded-full'>
                    <Icon size={10} />
                    {label}
                </div>
                <div className='absolute bottom-3 left-3 z-20 flex items-center gap-1
            bg-white border border-zinc-200 text-zinc-700 text-[10px] font-bold px-2.5 py-1.5 rounded-full shadow-sm'>
            ...
            <div className='flex flex-col flex-1 p-5 gap-4'>
                <div className='flex items-start justify-between gap-3'>
                    <div className='min-w-0'>
                        <h3 className='text-zinc-900 text-base font-black tracking-tight
                    leading-tight truncate'>{vehicle.vehicleModel}</h3>
                        <div className="mt-1.5 inline-flex items-center bg-zinc-100 px-2.5
                    py-1 rounded-lg border border-zinc-200">
                            <span className='text-zinc-500 text-xs font-black tracking-[0.2em] font-mono uppercase'>{vehicle.numberPlate}</span>
                        </div>
                    </div>
                    <div className='flex-shrink-0 w-10 h-10 rounded-2xl bg-zinc-100 border
                ...
                <div className='grid grid-cols-2 gap-2'>
                    <div className='bg-zinc-50 border border-zinc-100 rounded-2xl
                px-3.5 py-3'>
                        <div className='flex items-center gap-1.5 mb-1'>
                            <Gauge size={11} className='text-zinc-400' />
                            <p className='text-zinc-400 text-[9px] uppercase
                    tracking-widest font-bold'>Per km</p>
                        </div>
                    ...
                    <div className='bg-zinc-50 border border-zinc-100 rounded-2xl px-3.5 py-3'>
                        <div className='flex items-center gap-1.5 mb-1'>
                            ...
                            <p className='text-zinc-400 text-[9px] uppercase tracking-widest font-bold'>Waiting charge</p>
                        </div>
                        <div className='text-zinc-900 text-sm flex items-center font-black'>
                ...
                <div className='flex items-end justify-between pt-3 border-t border-zinc-100'>
                    ...
                    <motion.button
                        ...
                        onClick={onBook}
                        className="flex items-center gap-2 bg-zinc-900 hover:bg-black
            text-white text-sm font-black px-6 py-3.5 rounded-2xl transition-colors shadow-md"
                    >
                        Book
                        <motion.div
                            initial={{ x: 0 }}
