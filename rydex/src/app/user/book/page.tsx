...
const VEHICLES = [
  {id:"bike", label:"Bike", Icon:Bike, desc:"Quick & affordable rides" },
  {id:"car", label:"Car", Icon:Car, desc:"Fast & comfortable rides" },
  {id:"auto", label:"Auto", Icon:Car, desc:"Everyday rides" },
  {id:"laoding", label:"Loading Vehicle", Icon:Truck, desc:"For transporting goods" },
...
function page() {
  ...
  return (
    <div className='min-h-screen bg-zinc-100 flex items-center justify-center px-4 py-10'>
      ...
      >
        ...
        <div className='flex items-center gap-1.5 flex-shrink-0'>
          {
            [0,1,2,3].map((d,i)=>(
              <motion.div 
              key={i}
              animate={{width: i < progress ? 20 : 8, background : i < progress ? "#09090b" :
                "#d4d4d8"}}
        ...
        <div className='bg-white rounded-3xl border border-zinc-200 shadow-[0_8px_40px_rgba(0,0,0,0.08)]
        overflow-visible'>
              ...
              <div className='grid grid-cols-2 gap-2.5'>
                {VEHICLES.map((v,i)=>{
                  ...
                  return (
                   ...
                   whileTap={{scale:0.95}}
                   onClick={()=>setVehicle(v.id as vehicleTypes)}
                   className={`relative p-3.5 rounded-2xl border flex items-center gap-3 text-left transition-all
                    duration-200 ${active ? "bg-zinc-900 border-zinc-900 shadow-lg"
                      : "bg-zinc-50 border-zinc-200 hover:bg-zinc-400"
                    }`}
                   ...
                   >
                    ...
                    <div className='min-w-0'>
                      <p className={`text-sm font-bold truncate ${active ? "text-white" :
                        "text-zinc-900"}`}>{v.label}</p>
                      <p className={`text-[10px] truncate ${active ? "text-zinc-400" : 
                        "text-zinc-400"}`}>{v.desc}</p>
                    </div>
                    <motion.div
                    initial={{scale:0}}
                    animate={{scale:1}}
        ...
        <motion.div
            ...
            >
              ...
              <div className='flex items-center gap-3 bg-zinc-50 border-zinc-200 rounded-2xl
              px-4 py-3 focus-within:border-zinc-900 focus-within:bg-white transition-all'>
                ...
                placeholder:text-zinc-400 outline-none'
                 ...
                 <AnimatePresence>
                  ...
                  <p className='text-zinc-400 text-[10px] mt-1.5 ml-1'>Ride updates will be sent to this mobile number</p>
            </motion.div>
            ...
            >
              ...
              <div className='bg-zinc-50 border border-zinc rounded-2xl overflow-visible'>
                <div className='relative z-30'>
                  <div className='flex items-center gap-3 px-4 py-3.5 focus-within:bg-white rounded-t-2xl transition-colors'>
                    ...
                    disabled={locating}
                    className='w-8 h-8 rounded-xl bg-zinc-200 hover:bg-zinc-300
                      ...
                      >
                        {pickUpSuggestions.map((p,i)=>(
                          ...
                          >
                            ...
                            <ChevronRight size={13} className='text-zinc-300 flex-shrink-0 mt-auto'/>
                            </motion.div>
                        ))}
  ...
  router.push(
    `/user/search?pickup=${encodeURIComponent(pickUp)}&drop=${encodeURIComponent(drop)}&mobile=${encodeURIComponent(mobile)}&vehicle=${vehicle}&pickupLat=${pickUpLat}&pickupLon=${pickUpLon}&dropLat=${dropLat}&dropLon=${dropLon}`
            ...
            className='w-full h-14 rounded-2xl bg-zinc-900 hover:bg-black disabled:opacity-3
            text-white font-black text-sm tracking-wide flex items-center justify-center gap-25
