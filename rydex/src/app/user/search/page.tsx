...
import { ArrowLeft, Bus, MapPin, Navigation, Truck, Car, Bike, Zap, Search } from 'lucide-react';
  ...
  return (
    <div className='min-h-screen bg-zinc-100 text-zinc-900 overflow-x-hidden'>
        <div className='absolute top-5 left-5 z-50'>
            ...
            whileTap={{scale:0.88}}
            onClick={()=>router.back()}
            className='w-11 h-11 rounded-full bg-white border border-zinc-200 shadow-md flex
            items-center justify-center hover:bg-zinc-50 transition-colors'
            >
          ...
          >
            <div>
              ...
              </h2>
              {
                meta && <div className="text-zinc-400 text-xs mt-0.5">
                  {meta.label} rides near your location
                </div>
              }
            ...
            <AnimatePresence mode="wait">
              {loading ? (
                ...
                >
                  ...
                  border-t-zinc-700 animate-spin'/>
              ...
              vehicles.length>0 ?(
                ...
                initial={{opacity:0}}
                animate={{opacity:1}}
                className="flex items-center gap-1.5 bg-emerald-50 border border-emerald-200
                px-3 py-1.5 rounded-full"
                >
                  <Zap size={11} className="text-emerald-600 flex-emerald-600"/>
           ...
           <AnimatePresence>
              {!loading && vehicles.length===0 && (
                ...
                >
                  <div className='w-20 h-20 rounded-full bg-zinc-100 border border-zinc-200 flex items-center
                  justify-center mb-4'>
                    <Search size={26} className='text-zinc-400'/>
                  </div>
                  <p className='text-base text-zinc-900 font-bold mb-1'>Vehicles Not Found</p>
                  <p className='text-zinc-400 text-sm max-w-xs leading-relaxed'>
                    {meta?.label || "Vehicle"} drivers are visible nearby your location right now.
                  </p>
                  <motion.button
                  whileTap={{scale:0.95}}
                  onClick={()=>getNearByVehicles(pickUpLat,pickUpLon,vehicle)}
                  className='mt-5 flex items-center gap-2 bg-zinc-900 text-white
                  text-sm font-semibold px-6 py-2.5 rounded-xl hover:bg-zinc-800 transition-colors'
                  >
