...
function Page() {
    ...
    const [rLaoding, setRLoading] = useState(false);
    ...
    return (
        <div className="min-h-screen bg-black text-white flex flex-col">
            {/* Header */}
            <div className="px-6 py-4 border-b border-white/10 flex flex-col
      sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    <Image src={"/logo.jpeg"} alt="Logo" width={44} height={44} priority />
                    <p className="text-xs text-gray-400">{userData?.role == "admin" ? "Admin Verfication" : "Partner Video KYC"}</p>
                </div>
                {joined && (
          ...
          text-sm flex items-center gap-2"
            ...
            text-sm flex items-center gap-2"
                            onClick={() => router.push("/")}><PhoneOff size={16} />End Call</button>
                    </div>
                )}
                    ...
                    <div className="h-full flex items-center justify-center px-4 py-10">
                        <div className="w-full max-w-6xl grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
                            <div className="relative rounded-2xl overflow-hidden border border-white/10 bg-white/5">
                                <video ref={previewRef} autoPlay
                                    playsInline
                                    className="w-full h-[300px] sm:h-[400px] object-cover" />
                                {!isCameraOn && (
                                    <div className="absolute inset-0 bg-black flex items-center justify-center">
                                        <VideoOff size={40} /></div>)}
                            </div>
                    ...
                    >
                        <motion.div
                            ...
                            <div className="flex gap-4">
                                ...
                                <button className="flex-1 bg-green-600 rounded-xl py-2"
                                    disabled={aLoading}
                                    onClick={handleApprove}>{aLoading ? "Processing..." : "Approve"}</button>
                    ...
                    >
                        <motion.div
                            ...
                            <div className="flex gap-4">
                                ...
                                <button className="flex-1 bg-green-600 rounded-xl py-2"
                                    disabled={rLaoding}
                                    onClick={handleReject}>{rLaoding ? "Processing..." : "Reject"}</button>
